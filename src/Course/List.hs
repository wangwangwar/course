...
filter ::
  ...
  -> List a
  -> List a
filter f = foldRight (\c r -> if f c then c :. r else r) Nil
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = foldRight seq' $ Full Nil

seq' :: Optional a -> Optional (List a) -> Optional (List a)
seq' Empty _ = Empty
seq' _ Empty = Empty
seq' (Full x) (Full l) = Full $ x :. l
...
lengthGT4 ::
  List a
  -> Bool
-- Can't directly use 'length' because it can't handle infinity condition
